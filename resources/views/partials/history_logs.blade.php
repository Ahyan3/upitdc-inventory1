<div class="overflow-x-auto">
    <table class="min-w-full table-auto divide-y divide-[#ffcc34]" aria-label="History Logs">
        <thead class="table-header">
            <tr>
                <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-[#00553d] uppercase tracking-wider min-w-[150px]">Staff Name</th>
                <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-[#00553d] uppercase tracking-wider min-w-[100px]">Action</th>
        ...
        <tbody id="historyTableBody" class="bg-white divide-y divide-[#ffcc34]">
            @if ($history_logs->isEmpty())
                <tr>
                    <td colspan="7" class="px-4 py-4 text-center text-xs text-black bg-gradient-to-br from-gray-50 to-white rounded-lg border-2 border-dashed border-gray-300">
            ...
            @else
                @foreach ($history_logs as $log)
                    <tr class="hover:bg-gray-50 transition-colors slide-up">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-xs text-black min-w-[120px]">{{ $log->model_brand }} (ID: {{ $log->model_id }})</td>
...
<div id="historyPagination" class="pagination-container mt-4">
    <select id="per-page-display" class="bg-white border border-[#ffcc34] rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-[#00553d] mr-2">
        <option value="20" {{ $perPage == 20 ? 'selected' : '' }}>20</option>
        <option value="50" {{ $perPage == 50 ? 'selected' : '' }}>50</option>
        <option value="100" {{ $perPage == 100 ? 'selected' : '' }}>100</option>
    </select>
    <span class="pagination-info">
        Page {{ $history_logs->currentPage() }} to {{ $history_logs->currentPage() }} of {{ $history_logs->total() }} results
    </span>
    ...
    @foreach ($history_logs->getUrlRange(1, $history_logs->lastPage()) as $page => $url)
        <a href="{{ $url }}" class="pagination-btn {{ $history_logs->currentPage() == $page ? 'current' : '' }}">{{ $page }}</a>
    @endforeach
    ...
    <input type="number" id="historyPageJump" class="page-jump-input" placeholder="Page"
        min="1" max="{{ $history_logs->lastPage() }}"
        value="{{ $history_logs->currentPage() }}">
</div>